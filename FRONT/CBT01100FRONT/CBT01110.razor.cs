...
{
    internal partial class CBT01120 : R_Page
    {
        private CBT01100ViewModel _JournalListViewModel = new();
        private CBT01110ViewModel _JournalEntryViewModel = new();
        private R_Conductor _conductorRef;
        private R_ConductorGrid _conductorDetailRef;
        private R_Grid<CBT01111DTO> _gridDetailRef;
        ...
        protected override async Task R_Init_From_Master(object poParameter)
        {
            ...
            try
            {
                ...
                var loParam = R_FrontUtility.ConvertObjectToObject<CBT01110DTO>(poParameter);
        ...
        private async Task JournalForm_GetRecord(R_ServiceGetRecordEventArgs eventArgs)
        {
            ...
            try
            {
                await _JournalEntryViewModel.GetJournal((CBT01110DTO)eventArgs.Data);
        ...
        private async Task JournalForm_ServiceSave(R_ServiceSaveEventArgs eventArgs)
        {
            ...
            try
            {
                await _JournalEntryViewModel.SaveJournal((CBT01110DTO)eventArgs.Data, (eCRUDMode)eventArgs.ConductorMode);
                eventArgs.Result = _JournalEntryViewModel.Journal;
            }
            catch (Exception ex)
            {
        ...
        private async Task BtnDelete_OnClick()
        {
            ...
            try
            {
                ...
                var loData = (CBT01110DTO)_conductorRef.R_GetCurrentData();
        ...
        private async Task JournalForm_AfterAdd(R_AfterAddEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                var data = (CBT01110DTO)eventArgs.Data;
        ...
        private void ValidationFormGLT00100JournalEntry(R_ValidationEventArgs eventArgs)
        {
            ...
            try
            {
                var loParam = (CBT01110DTO)eventArgs.Data;
                if (string.IsNullOrWhiteSpace(loParam.CDEPT_CODE))
                {
                    //loEx.Add(R_FrontUtility.R_GetError(typeof(Resources_Dummy_Class),"V01"));
                ...
                else
                {
                    ...
                    if (int.Parse(_JournalEntryViewModel.RefDate.ToString("yyyyMMdd")) < int.Parse(_JournalEntryViewModel.VAR_CB_SYSTEM_PARAM.CCB_LINK_DATE))
                    {
                        //loEx.Add(R_FrontUtility.R_GetError(
                        //    typeof(Resources_Dummy_Class),
                        //    "V05"));
                        loEx.Add("", "Reference Date cannot be before CB Link Date!");

                    }

                    if (int.Parse(_JournalEntryViewModel.RefDate.ToString("yyyyMMdd")) < int.Parse(_JournalEntryViewModel.VAR_CCURRENT_PERIOD_START_DATE.CEND_DATE))
                    {
                        //loEx.Add(R_FrontUtility.R_GetError(
                        //    typeof(Resources_Dummy_Class),
                        //    "V06"));
                        loEx.Add("", "Reference Date cannot be before Soft Period!");
                ...
                if (string.IsNullOrWhiteSpace(loParam.CCB_CODE))
                {
                    ...
                    //    typeof(Resources_Dummy_Class),
                    //    "V07"));
                    loEx.Add("", "Please select Bank Code!");
                ...
                if (loParam.NTRANS_AMOUNT <= 0)
                {
                    ...
                    //    typeof(Resources_Dummy_Class),
                    //    "V08"));
                    loEx.Add("", "Receipt Amount must be > 0!");
                ...
                {
                    if (int.Parse(_JournalEntryViewModel.DocDate.Value.ToString("yyyyMMdd")) > int.Parse(_JournalEntryViewModel.RefDate.ToString("yyyyMMdd")))
                    {
                        //loEx.Add(R_FrontUtility.R_GetError(
                        //    typeof(Resources_Dummy_Class),
                        //    "V11"));
                        loEx.Add("", "Document Date cannot be after Reference Date!");

                    }

                    if (int.Parse(_JournalEntryViewModel.DocDate.Value.ToString("yyyyMMdd")) < int.Parse(_JournalEntryViewModel.VAR_CCURRENT_PERIOD_START_DATE.CEND_DATE))
                    {
                        //loEx.Add(R_FrontUtility.R_GetError(
                        //    typeof(Resources_Dummy_Class),
                        //    "V12"));
                        loEx.Add("", "Document Date cannot be before Soft Period!");
        ...
        private async Task CopyJournalEntryProcess()
        {
            ...
            try
            {
                var loOldData = (CBT01110DTO)_conductorRef.R_GetCurrentData();
                await _conductorRef.Add();
                if (_conductorRef.R_ConductorMode == R_eConductorMode.Add)
                {
                    var loNewData = (CBT01110DTO)_conductorRef.R_GetCurrentData();
        ...
        public async Task RefreshCurrency()
        {
            ...
            {
                var loData = (CBT01110DTO)_conductorRef.R_GetCurrentData();
                var loParam = R_FrontUtility.ConvertObjectToObject<CBT01110LastCurrencyRateDTO>(loData);

                var loResult = await _JournalEntryViewModel.GetLastCurrency(new CBT01110LastCurrencyRateDTO()
                {
                    CCURRENCY_CODE = loData.CCURRENCY_CODE,
                    CRATETYPE_CODE = _JournalEntryViewModel.VAR_GL_SYSTEM_PARAM.CRATETYPE_CODE,
                    CRATE_DATE = _JournalEntryViewModel.RefDate.ToString("yyyyMMdd")
                });
        ...
        private void JournalDet_RDisplay(R_DisplayEventArgs eventArgs)
        {
            ...
            try
            {
                var data = (CBT01111DTO)eventArgs.Data;
                var loHeaderData = (CBT01110DTO)_conductorRef.R_GetCurrentData();
        ...
        private void JournalDet_AfterAdd(R_AfterAddEventArgs eventArgs)
        {
            ...
            {
                //var loHeaderData = (CBT01110DTO)_conductorRef.R_GetCurrentData();
                //var loData = (CBT01110DTO)eventArgs.Data;
        ...
        //{
        //    if (eventArgs.ColumnName == nameof(CBT01110DTO.CGLACCOUNT_NO))
        //    {
        ...
        //    }
        //    else if (eventArgs.ColumnName == nameof(CBT01110DTO.CCASH_FLOW_CODE))
        //    {
        ...
        //{
        //    CBT01110DTO loGetData = (CBT01110DTO)eventArgs.ColumnData;
        //    if (eventArgs.ColumnName == nameof(CBT01110DTO.CGLACCOUNT_NO))
        //    {
