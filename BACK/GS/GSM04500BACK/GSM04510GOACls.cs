...
{
    public class GSM04510GOACls : R_BusinessObject<GOADTO>
    {
        ...
        protected override void R_Deleting(GOADTO poEntity)
        {
            throw new NotImplementedException();
        }

        protected override GOADTO R_Display(GOADTO poEntity)
        {
            ...
            try
            {
                ...
                loReturn = R_Utility.R_ConvertTo<GOADTO>(loReturnTemp).ToList().FirstOrDefault();
            }
            catch (Exception ex)
            {
                loException.Add(ex);
        ...
        protected override void R_Saving(GOADTO poNewEntity, eCRUDMode poCRUDMode)
        {
        ...
        public List<GOADTO> JOURNAL_GROUP_GOA_LIST(GSM04510GOADBParameter poParameter)
        {
            string lcMethodName = nameof(JOURNAL_GROUP_GOA_LIST);
            _loggerGSM04500.LogInfo(string.Format("START process method {0} on Cls", lcMethodName));
            R_Exception loException = new R_Exception();
            List<GOADTO> loReturn = null;
            R_Db loDb;
            DbCommand loCommand;
            ...
            try
            {
                ...
                loReturn = R_Utility.R_ConvertTo<GOADTO>(loReturnTemp).ToList();
